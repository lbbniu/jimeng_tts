...
class ImageStorage:
    ...
    def _init_db(self):
        ...
        try:
            ...
            conn.commit()
            conn.close()
            logger.info("[Jimeng] Database initialized")
            
        except Exception as e:
            logger.error(f"[Jimeng] Failed to initialize database: {e}")
            raise e
    ...
    def store_image(self, img_id: str, urls: list, metadata: dict = None):
        ...
        try:
            ...
            conn.commit()
            conn.close()
            logger.debug(f"[Jimeng] Stored image {img_id}")
            
        except Exception as e:
            logger.error(f"[Jimeng] Failed to store image {img_id}: {e}")
            raise e
    ...
    def get_image(self, img_id: str) -> dict:
        ...
        try:
            ...
            logger.error(f"[Jimeng] Failed to get image {img_id}: {e}")
            return None
    ...
    def delete_image(self, img_id: str):
        ...
        try:
            ...
            conn.commit()
            conn.close()
            logger.debug(f"[Jimeng] Deleted image {img_id}")
            
        except Exception as e:
            logger.error(f"[Jimeng] Failed to delete image {img_id}: {e}")
    ...
    def cleanup_expired(self):
        ...
        try:
            ...
            conn.commit()
            conn.close()
            logger.info("[Jimeng] Cleaned up expired images")
            
        except Exception as e:
            logger.error(f"[Jimeng] Failed to cleanup expired images: {e}") 